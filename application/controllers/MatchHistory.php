...
class MatchHistory extends CI_Controller {
	...
	public function ajax_manage_page()
	{
		...
		{
			// $btn = '';
			// $btn = ''.anchor(site_url(MACTHHISTORYVIEW.'/'.base64_encode($userData->tableId)),'<span title="View" class="btn btn-primary btn-circle btn-xs"  data-placement="right" title="View"><i class="fa fa-eye"></i></span>');
			$table='';
	...
	public function exportAction(){
		...
		if(!empty($getUserData)) {
			...
			foreach ($getUserData as $report) {
				...
				$cond= "tableId='".$report->tableId."'";
				$getData = $this->MatchHistory_model->matchesData($cond);

				$this->excel->getActiveSheet()->setCellValue('A'.$a, $sr);
				$this->excel->getActiveSheet()->setCellValue('B'.$a, $report->tableId);
				$this->excel->getActiveSheet()->setCellValue('C'.$a, $report->gameType);
				$this->excel->getActiveSheet()->setCellValue('D'.$a, $report->betValue);
				...
				foreach ($getData as $record) {
					...
					$this->excel->getActiveSheet()->setCellValue('H'.$b, $record->coins);
