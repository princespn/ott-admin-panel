        ...
        <div class="col-xs-12">
          ...
          <div class="box " style="border-top:2px solid red; margin-top: 10px;">
            ...
            <div class="box-body" >
              ...
              <?php if(!empty($getSetting->maintainance) && $getSetting->maintainance=='No') { ?>
							<button type="button" class="btn btn-danger" onclick="return openMaintainance('<?= $getSetting->id; ?>','<?= $getSetting->maintainance; ?>')"><?= $getSetting->maintainance; ?></button>
            <?php }else{ ?>
                <button type="button" class="btn btn-success"  onclick="return openMaintainance('<?= $getSetting->id; ?>','<?= $getSetting->maintainance; ?>')"><?= $getSetting->maintainance; ?></button>
            <?php } ?>
						</div>
					</div>
                  </div>
         ...
         <div class="modal-dialog">
            <div class="modal-content">
               <div class="modal-header">
                  ...
                  <h4 class="modal-title">Maintainance Mode</h4>
               </div>
               <div class="modal-body">
                  <input type="hidden" name="userId" id="userId">
                  <div class="form-group">
                     <label for="mode">Maintainance Mode<span class="text-danger"> * </span><span id="err_mode" class="text-danger"></span></label>
                  ...
                  <div class="form-group">
                     <button type="button" class="btn btn-success" onclick="maintainance()">Submit</button>
                  </div>
               </div>
...
<script type="text/javascript">
  ...
  { 
    ...
    /*if(mode == '')
    {
      ...
      var url = site_url+"/Maintainance/maintainanceUpdate";
      var datastring = "id="+userId+"&mode="+mode+"&maintainanceMsg="+maintainanceMsg+"&"+csrfName+"="+csrfHash;
