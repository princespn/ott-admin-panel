...
class CompletedRequest_model extends CI_Model
{
	var $column_order = array('ud.user_name','ua.orderId','ua.amount','ua.paymentMode',null,null); //set column field database for datatable orderable
    var $column_search = array('ud.user_name','ua.orderId','ua.amount','ua.paymentMode'); //set column field database for datatable searchable 
    var $order = array('ua.created' => 'DESC'); 
    ...
    function getExportData($table,$condition=''){
        ...
        $sql="SELECT `ua`.*, `u`.`user_name`, `u`.`user_id`, `u`.`user_name`, `u`.`email_id`, `u`.`mobile`, `bd`.`acc_holderName`, `bd`.`accno`, `bd`.`ifsc` FROM `user_deposit_withdraw` `ua` LEFT JOIN `user_details` `u` ON `u`.`id`=`ua`.`userId` LEFT JOIN `bank_details` `bd` ON `bd`.`user_detail_id`=`ua`.`userId`  ";
        if(!empty($condition)){
            $sql .= "  where $condition ";
        }
