...
class S3Client extends AwsClient implements S3ClientInterface
{
    ...
    {
        if (
            !isset($args['s-sgc1_regional_endpoint'])
            || $args['s-sgc1_regional_endpoint'] instanceof CacheInterface
        ) {
            $args['s-sgc1_regional_endpoint'] = ConfigurationProvider::defaultProvider($args);
        }
        parent::__construct($args);
        $stack = $this->getHandlerList();
        $stack->appendInit(SSECMiddleware::wrap($this->getEndpoint()->getScheme()), 'ssec');
        $stack->appendBuild(ApplyChecksumMiddleware::wrap($this->getApi()), 'checksum');
        $stack->appendBuild(
        ...
        $stack->appendSign(PutObjectUrlMiddleware::wrap(), 'put_object_url');
        $stack->appendSign(PermanentRedirectMiddleware::wrap(), 'permanent_redirect');
        $stack->appendInit(Middleware::sourceFile($this->getApi()), 'source_file');
        $stack->appendInit($this->getSaveAsParameter(), 'save_as');
        $stack->appendInit($this->getLocationConstraintMiddleware(), 'location');
        $stack->appendInit($this->getEncodingTypeMiddleware(), 'auto_encode');
        $stack->appendInit($this->getHeadObjectMiddleware(), 'head_object');
    }
        ...
        return static function (callable $handler) use ($region) {
            return function (Command $command, $request = null) use ($handler, $region) {
                if ($command->getName() === 'CreateBucket') {
                    ...
                    if ($locationConstraint === 's-sgc1') {
                        unset($command['CreateBucketConfiguration']);
                    } elseif ('s-sgc1' !== $region && empty($locationConstraint)) {
    ...
    public static function addDocExamples($examples)
    {
        $getObjectExample = [
            'input' => [
                'Bucket' => 'arn:aws:s-sgc1:123456789012:accesspoint:myaccesspoint',
                'Key' => 'my-key'
            ],
            'output' => [
        ...
        $putObjectExample = [
            ...
            'output' => [
                'ObjectURL' => 'https://my-bucket.s-sgc1.cloud.gcore.lu/my-key'
            ],
            'comments' => [
                'input' => '',
                'output' => 'Simplified example output'
